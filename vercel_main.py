...
async def match_jobs_with_resume(request: MatchJobsRequest):
    ...
    try:
        ...
        logger.info(f"Starting job matching on platform: {request.platform}")

        # Get platform handler and search jobs
        platform_handler = PlatformHandlerFactory.get_handler(request.platform, loader)
        jobs = platform_handler.extract_jobs(
            request.job_search_keyword,
            request.job_search_location
        )
